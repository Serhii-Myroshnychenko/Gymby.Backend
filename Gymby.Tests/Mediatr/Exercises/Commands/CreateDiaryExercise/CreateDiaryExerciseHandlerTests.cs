...
namespace Gymby.UnitTests.Mediatr.Exercises.Commands.CreateDiaryExercise
{
    public class CreateDiaryExerciseHandlerTests
    {
        ...
        [Fact]
        public async Task CreateDiaryExerciseHandler_NonexistentDiary_ShouldBeFail()
        {
            // Arrange
            var handlerProgram = new CreateProgramHandler(Context, Mapper);
            var handlerProgramDay = new CreateProgramDayHandler(Context, Mapper);
        ...
        public async Task CreateDiaryExerciseHandler_NonexistentExersicePrototype_ShouldBeFail()
        {
            ...
            Assert.Equal($"Entity \"{exercisePrototypeId}\" ({nameof(Domain.Entities.ExercisePrototype)}) not found", exception.Message);
        }
    }
}
